...
# FIX 1: Wrapped the MCP initialization in a proper tenacity retry block
@retry(
...
async def sentiment_agent_node(state: FinancialSwarmState) -> dict:
    ...
    try:
        ...
        # FIX 2: Properly nested the data under the ticker key so the Orchestrator can actually read it
        return {
            "sentiment_data": {ticker: final_data}
        }
